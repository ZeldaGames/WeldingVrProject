...
public class WeldingHandle : MonoBehaviour
{
    ...
    private void ShowBlob()
    {
        ...
        if (!hasBlob)
        {
            if (weldHit.transform.gameObject.layer == 8) //Hit is panel
            {
            ...
            else if (weldHit.transform.gameObject.layer == 9) //Hit is Blob
            {
    ...
    public Vector3 GetWeldPoint()
    {
        ...
        if (Physics.Raycast(weldingTip.position, weldingTip.forward, out RaycastHit hit, rayLength))
        {
            ...
            if (hit.transform.gameObject.layer == 8)
            {
                ...
                Debug.Log("Weld layer " + hit.transform.gameObject.name);
            }
            else
            {
                weldHit = new RaycastHit();
                isWeldingLayer = false;
                Debug.Log("Not Weld layer " + hit.transform.gameObject.name);
            }
            ...
            Debug.DrawRay(
                weldingTip.position,
                weldingTip.forward * hit.distance,
                Color.red
            );
        }
        else
        {
            // Draw full ray when nothing is hit
            Debug.DrawRay(
                weldingTip.position,
                weldingTip.forward * rayLength,
                Color.green
            );
        }
        ...
        {
            if (hit2.transform.gameObject.layer == 8 && hit2.distance < 0.02f)
            {
                hasFrontPanel = true;
    ...
    /// <summary>
    /// Safe getter for the current weld hit info
    /// </summary>
    public bool TryGetWeldHit(out RaycastHit hit)
    {
        hit = weldHit;        // weldHit is already stored internally
        return isWeldingLayer; // returns true if we are hitting a weldable panel
    }
